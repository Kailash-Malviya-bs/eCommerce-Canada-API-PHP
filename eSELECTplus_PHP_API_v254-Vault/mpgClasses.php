...
class mpgGlobals
{
	var $Globals=array(
        	        ...
        	        MONERIS_PORT =>'443',
                  	...
                  	API_VERSION  =>'PHP - 2.5.4 - Resolver',
                  	CLIENT_TIMEOUT => '60'
                 	);
...
class mpgHttpsPostStatus
{
 	...
 	function mpgHttpsPost($storeid,$apitoken,$status,$mpgRequestOBJ)
 	{
		...
		//echo "String to be sent = " . $dataToSend;
...
class mpgRequest
{
 	...
 	var $txnTypes =array(
			...
			res_add_cc => array('cust_id','phone','email','note','pan','expdate','crypt_type','data_key'),
	...
	function toXML()
	{
 		...
 		for($x=0;$x < $txnArrayLen;$x++)
 		{
			...
			$dataKeyFormat = $txnObj->getDataKeyFormat();
			if($dataKeyFormat != null)
			{
				$txnXMLString .= "<data_key_format>".$dataKeyFormat."</data_key_format>";
			}

			$dataKey = $txnObj->getDataKey();
			if($dataKey != null)
			{
				$txnXMLString .= "<data_key>".$dataKey."</data_key>";
			}
...
class mpgTransaction
{
	...
	function getDataKeyFormat()
	{
		return $this->data_key_format;
	}

	function setDataKeyFormat($dataKeyFormat)
	{
		$this->data_key_format = $dataKeyFormat;
	}
