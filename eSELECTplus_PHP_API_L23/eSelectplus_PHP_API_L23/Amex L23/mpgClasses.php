...
var $Globals=array(
                  MONERIS_PROTOCOL => 'http',
                  MONERIS_HOST => 'localhost',
                  MONERIS_PORT =>'80',
...
class mpgHttpsPost{
 ...
 function mpgHttpsPost($storeid,$apitoken,$mpgRequestOBJ)
 {
  ...
  if ( $transactionType == "mccompletion" || $transactionType == "mcrefund"
       || $transactionType == "mcind_refund" || $transactionType == "mcpurchasecorrection"
       || $transactionType == "mcforcepost" || $transactionType == "vslevel23"
       || $transactionType == "mclevel23" || $transactionType == "vspurchal" )
  {
    $gArray[MONERIS_FILE]="/level23/level23servlet";
  }
  elseif ( $transactionType == "axcompletion" || $transactionType == "axrefund"
       || $transactionType == "axind_refund" || $transactionType == "axpurchasecorrection"
       || $transactionType == "axforcepost"  ) {
    $gArray[MONERIS_FILE]="/level23/axlevel23servlet";
  }
...
function getMessageId(){
 ...
 return ($this->responseData['MessageId']);
...
class mpgRequest{
 ...
 var $txnTypes =array(purchase=> array('order_id','cust_id', 'amount', 'pan', 'expdate', 'crypt_type'),
                      ...
                      batchcloseall => array() ,
                      mccompletion => array('order_id', 'comp_amount','txn_number', 'crypt_type'),
                      mcforcepost => array('order_id','cust_id', 'amount','pan','expdate','auth_code', 'crypt_type'),
                      mcind_refund => array('order_id', 'amount','pan','expdate', 'crypt_type'),
                      mcrefund => array('order_id', 'amount', 'txn_number', 'crypt_type'),
                      mcpurchasecorrection => array('order_id', 'txn_number', 'crypt_type'),
                      vscompletion => array('order_id', 'comp_amount','txn_number', 'crypt_type','order_level_gst','merchant_gst_no','cri'),
                      vsrefund => array('order_id', 'amount','txn_number', 'crypt_type','order_level_gst','merchant_gst_no','cri'),
                      vsind_refund => array('order_id', 'amount','pan','expdate', 'crypt_type','order_level_gst','merchant_gst_no','cri'),
                      vsforcepost => array('order_id', 'cust_id','amount','pan','expdate','auth_code', 'crypt_type','order_level_gst','merchant_gst_no','cri'),
                      vslevel23 => array('order_id', 'txn_number'),
                      mclevel23 => array('order_id', 'txn_number'),
                      vspurchal => array('order_id', 'txn_number'),
                      axcompletion => array('order_id', 'comp_amount','txn_number'),
                      axforcepost => array('order_id', 'amount','pan','expdate','auth_code'),
                      axind_refund => array('order_id', 'amount','pan','expdate'),
...
function toXML(){
 ...
 for($x=0;$x < $txnArrayLen;$x++)
 {
   ...
   $gxmlString = $txnObj->getGxmlString();
   if ( $gxmlString != null )
   {
     $txnXMLString .= $gxmlString;
   }

   $mcLevel23String = $txnObj->getMcLevel23String();
   if ( $mcLevel23String != null )
   {
     $txnXMLString .= $mcLevel23String;
   }

   $axLevel23String = $txnObj->getAxLevel23String();
   if ( $axLevel23String != null )
   {
     $txnXMLString .= $axLevel23String;
   }

   $vsPurchalString = $txnObj->getVsPurchalString();
   if ( $vsPurchalString != null )
   {
     $txnXMLString .= $vsPurchalString;
   }
...
class mpgTransaction{
 ...
 var $recur = null;
 var $gxmlString = null;
 var $mcLevel23String = null;
 var $axLevel23String = null;
 var $vsPurchalString = null;
...
function setGxmlString($gxmlString)
{
  $this->gxmlString = $gxmlString;
}
function getGxmlString()
{
  return $this->gxmlString;
}
function setMcLevel23String($mcLevel23String)
{
  $this->mcLevel23String = $mcLevel23String;
}
function getMcLevel23String()
{
  return $this->mcLevel23String;
}
function setAxLevel23String($axLevel23String)
{
  $this->axLevel23String = $axLevel23String;
}
function getAxLevel23String()
{
  return $this->axLevel23String;
}
function setVsPurchalString($vsPurchalString)
{
  $this->vsPurchalString = $vsPurchalString;
}
function getVsPurchalString()
{
  return $this->vsPurchalString;
}
